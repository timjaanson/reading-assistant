...
export const ChatTab = () => {
  ...
  const saveChat = useCallback(async () => {
    ...
    try {
      ...
      if (currentChatId) {
        ...
        console.log(`Created new chat with ID:${id} and name:${chatName}`);
        setCurrentChatId(id);
      }
  ...
  // New useEffect to auto-generate chat name when an assistant response is present in a new chat
  // useEffect(() => {
  //   if (
  //     chatName === "New Chat" &&
  //     !chatNameAutoGenerated &&
  //     messages.length > 0 &&
  //     messages.length <= 2 &&
  //     currentChatId !== null
  //   ) {
  //     (async () => {
  //       let contentForNameGeneration = messages
  //         .map((m) =>
  ...
  //       if (contentForNameGeneration.length > 400) {
  //         contentForNameGeneration =
  //           contentForNameGeneration.slice(0, 400) + "...";
  //       }

  //       console.log(
  //         "Auto-generating chat name based on:",
  //         contentForNameGeneration
  //       );
  //       try {
  //         const { text } = await getSyncTextResponse(
  //           [{ role: "user", content: contentForNameGeneration }],
  //           { systemPrompt: generateChatNameSystemMessage() }
  //         );
  //         console.log("Generated chat name response:", text);
  //         const match = text.match(/<chat_name>(.*?)<\/chat_name>/);
  //         if (match && match[1]) {
  //           const newGeneratedName = match[1].trim();
  //           console.log("Auto-generated chat name:", newGeneratedName);
  //           setChatName(newGeneratedName);
  //           await saveChat();
  //           setChatNameAutoGenerated(true);
  //         }
  //       } catch (error) {
  //         console.error("Error auto-generating chat name:", error);
  //       }
  //     })();
  //   }
  // }, [currentChatId, chatName, chatNameAutoGenerated, messages, saveChat]);
